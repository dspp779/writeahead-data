...
# You can refer to data.awl.bawe.pdf
...
pdev = set(map(lambda x: x.strip().split('\t', 1)[0], open('data.pdev.verbs.all.txt')))

# Academic Keyword List and Teufel Word List
# format: wordA, wordB,... 
# aklWords = re.findall('[a-z_]+', (open('data.akl.all.txt').read()+open('data.teufel.all.txt').read()).lower()))
aklWords = { word for words in map(lambda x:x.strip().split(', '), open('data.akl.all.txt')) for word in words}
teufelWords = { word.replace('_', ' ') for words in map(lambda x:x.strip().split(', '), open('data.teufel.all.txt')) for word in words}
aklWords = awl3000|longman3000|pdev|teufelWords|aklWords
